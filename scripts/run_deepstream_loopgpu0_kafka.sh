#!/bin/bash
PIPE_PATH="/tmp/stop_pipe"
if [ ! -p "$PIPE_PATH" ]; then
  mkfifo "$PIPE_PATH"
fi

OUTPUT_DIRS=(
  "/home/bb24902/runOutput/13MarOutput"
 # "/home/bb24902/gtOutput/24MarOutput"
 # "/home/bb24902/gtOutput/6MarOutput"
)
CONFIG_FILES=(
  "/home/bb24902/deepstream-fewshot-learning-app/configs/13mar/mtmc_config.txt"
...
for ((i=0; i<$NUM_TASKS; i++)); do
  ...
  ./deepstream-fewshot-learning-app -c "$CONFIG_FILE" -m 1 -t 1 -l 5 \
    --message-rate 1 --tracker-reid 1 --reid-store-age 25 -b 1 -p 1

  echo "DeepStream finished. Stopping Python Kafka consumer..."
  echo "exit" > /tmp/stop_pipe
